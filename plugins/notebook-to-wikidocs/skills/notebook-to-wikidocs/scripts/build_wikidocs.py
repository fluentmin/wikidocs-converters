...
분할(장→절):
  ...
  --split sections       : --config 의 section_rules/subpages 로 한 장을 여러 절로 분할.
                           (예: 실습/해부/변형/정리 — config/neuqes-101.json 참고)
...
사용:
  ...
  python3 build_wikidocs.py 07_bert_pipeline --root ~/proj --config config/neuqes-101.json --split sections
...
def main() -> None:
    ...
    ap.add_argument("--split", choices=("single", "sections"), default=None,
                    help="single(기본): 노트북=페이지 / sections: config 규칙으로 장→절 분할")
